...
const getDashboard = async (req, res) => {
  try {
    ...
    const balanceVariation = lastMonthBalance !== 0 
    ...
    // Estatísticas gerais
    const totalTransactions = await transactionModel.countDocuments({ userId: req.userId });
      ...
      currentMonth: {
        ...
        variations: {
          income: {
            ...
            trend: incomeVariation > 0 ? 'up' : incomeVariation < 0 ? 'down' : 'stable'
          },
          expense: {
            value: expenseVariation,
            trend: expenseVariation > 0 ? 'up' : expenseVariation < 0 ? 'down' : 'stable'
          },
          balance: {
            value: balanceVariation,
            trend: balanceVariation > 0 ? 'up' : balanceVariation < 0 ? 'down' : 'stable'
          }
        }
      },
      lastMonth: {
